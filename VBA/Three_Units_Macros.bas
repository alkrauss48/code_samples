...
Sub OwnerSummedTract_3Units()

'gets names of the 2 additional spreadsheets, and ends the program if user selects cancel
...
Dim thirdSheetName As String
thirdSheetName = InputBox("What is the title of the third spreadsheet?", "Input Third Spreadsheet")
If thirdSheetName = "" Then
    End
End If
...
Dim WS As Worksheet
Set WS = Sheets.Add
WS.Name = "Owner Summed Tract_3 Units"
current.Select
    Columns("A:C").Select
    Selection.Copy
...
Set thirdSheet = Sheets(thirdSheetName)    '*****INSERT THIRD SPREADSHEET NAME HERE*****'
thirdSheet.Activate                        '*********************************************'
...
Dim ThirdLastRow, LastRow111, LastRow222, LastRow333, LastRow444, LastRow555, LastRow666   'last rows for third sheet
LastRow111 = Range("R" & Rows.Count).End(xlUp).Row
LastRow222 = Range("S" & Rows.Count).End(xlUp).Row
LastRow333 = Range("T" & Rows.Count).End(xlUp).Row
LastRow444 = Range("U" & Rows.Count).End(xlUp).Row
LastRow555 = Range("V" & Rows.Count).End(xlUp).Row
LastRow666 = Range("W" & Rows.Count).End(xlUp).Row

ThirdLastRow = WorksheetFunction.Max(LastRow111, LastRow222, LastRow333, LastRow444, LastRow555, LastRow666)
...
Range("Q1").Value = thirdSheet.Range("A1")
Range("Q1").Select
Selection.AutoFill Destination:=Range("Q1:Q" & ThirdLastRow), Type:=xlFillDefault

'move data from second and third sheets underneath first sheet data
...
Range("Q1:W" & ThirdLastRow).Select
Selection.Copy
Range("A" & LastRow + OtherLastRow + 1).Select
Selection.PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks _
        :=False, Transpose:=False
        
Columns("I:W").Select                   'deletes unnecessary columns used in creation of this spreadsheet
...
LastRow3 = Range("D" & Rows.Count).End(xlUp).Row
LastRow4 = Range("E" & Rows.Count).End(xlUp).Row
LastRow5 = Range("G" & Rows.Count).End(xlUp).Row
LastRow6 = Range("I" & Rows.Count).End(xlUp).Row
...
End Sub
Sub SummedOwnerUpload_3Units()

'gets names of the additional spreadsheets, and ends the program if user selects cancel
...
Dim WS As Worksheet
Set WS = Sheets.Add
WS.Name = "Summed Owner Upload_3 Units"
current.Select
    Columns("B:C").Select
    Selection.Copy
    Sheets(WS.Name).Select
...
Dim ThirdLastRow, LastRow111, LastRow222, LastRow333, LastRow444, LastRow555  'last rows for third sheet
LastRow111 = Range("R" & Rows.Count).End(xlUp).Row
LastRow222 = Range("S" & Rows.Count).End(xlUp).Row
LastRow333 = Range("T" & Rows.Count).End(xlUp).Row
LastRow444 = Range("U" & Rows.Count).End(xlUp).Row
...
ThirdLastRow = WorksheetFunction.Max(LastRow111, LastRow222, LastRow333, LastRow444, LastRow555)
...
Range("Q1:V" & ThirdLastRow).Select
Selection.Copy
Range("A" & LastRow + OtherLastRow + 1).Select
Selection.PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks _
        :=False, Transpose:=False
...
Columns("I:V").Select                   'deletes unnecessary columns used in creation of this spreadsheet
Selection.Delete Shift:=xlToLeft
